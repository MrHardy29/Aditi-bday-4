...
// Cinematic memory video — only Play/Pause is visible.
const memoryVideo = document.getElementById("memoryVideo");
const videoPlay = document.getElementById("videoPlay");
const videoMemory = document.querySelector(".video-memory");
if (memoryVideo && videoPlay) {
  videoPlay.onclick = () => {
    if (memoryVideo.ended) memoryVideo.currentTime = 0;
    if (memoryVideo.paused) memoryVideo.play().catch(()=>{});
    else memoryVideo.pause();
  };
  memoryVideo.onplay = () => {
    videoPlay.textContent = "Ⅱ";
    videoPlay.setAttribute("aria-label","Pause video");
    videoMemory.classList.add("playing");
  };
  memoryVideo.onpause = () => {
    videoPlay.textContent = "▶";
    videoPlay.setAttribute("aria-label","Play video");
    videoMemory.classList.remove("playing");
  };
  memoryVideo.onended = () => {
    videoPlay.textContent = "↻";
    videoPlay.setAttribute("aria-label","Replay video");
    videoMemory.classList.remove("playing");
  };
}
